...
import { downloadUrl, formatBytes } from '../util';
...
export function UploadWidgetUploadItem({
	...
	return (
		<motion.div
			className='p-3 rounded-lg flex flex-col gap-3 shadow-content bg-white/[0.02] relative overflow-hidden'
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			transition={{ duration: 0.5 }}
		>
			<div className='flex flex-col gap-1'>
				<span className='text-xs font-medium flex gap-1 text-ellipsis overflow-hidden'>
					<ImageUp
			...
			<div className='absolute top-2 right-2 flex items-center gap-1'>
				<Button
					...
					onClick={() => {
						if (upload.remoteUrl) {
							downloadUrl(upload.remoteUrl, upload.name);
						}
					}}
				>
